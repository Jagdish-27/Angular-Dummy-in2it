...
export class OrganizationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   'Organization_Data',
    //   JSON.stringify(this.serverService.rowData)
    // );
  }

  setDataInitialy() {
    this.customerCount = this.serverService.rowData.filter(
      (data) => data.type === 'Customer'
    ).length;
    this.nonCustomerCount = this.serverService.rowData.filter(
      (data) => data.type === 'Non Customer'
    ).length;
  ...
  filterOrganization(value: string) {
    ...
    if (value === 'Customer') {
      this.filteredData = this.serverService.rowData.filter(
        (data) => data.type === 'Customer'
      );
  ...
  filterOrgData(searchValue: string): any[] {
    ...
    return this.serverService.rowData.filter((data: OrganizationData) => {
      return (
        ...
        data.orgSPOC.toLowerCase().includes(searchValue) ||
        data.type.toLowerCase().includes(searchValue) ||
        data.contacs.some((contact) =>
  ...
  colDefsTable: ColDef[] = [
    ...
    { headerName: 'Org SPOC', field: 'orgSPOC' },
    {
      headerName: 'Email',
      field: 'email',
      valueGetter: (params) => params.data.contacs[0].email,
    },
    {
      headerName: 'Phone',
      field: 'phone',
      valueGetter: (params) => params.data.contacs[0].phone,
    },
  ];
