...
export class SideFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(): void {
    if (this.item) {
      ...
      this.isEditable = this.gridOptions.context.componentParent.isEditable;
    } else {
  ...
  onForm_Submit() {
    if (this.task_form.valid) {
      const form_Data = {
        ...
        id: this.task_form.get('id')?.value,
      };
      if (this.isEditable) {
      ...
      key.map((val) => {
        let control =
          this.task_form.controls[val as keyof typeof this.task_form.controls];
        if (control.errors) {
